...
- **Preflight local**: Antes de cualquier publicación o commit grande estructural, se exige correr `npm run preflight` (que agrupa lint, tipos y auditorías de seguridad).
...
- Comandos: `npm run preflight` y `npm run build` aseguran que la app pase los estándares de calidad locales antes de considerar subirla.
