...
| 2026-07-14 22:20 | Codex | Inicia ejecución continua de Fases 5 y 6. Fase 5 reclama exclusivamente analítica, una migración nueva y documentación para origen/rutas/bots, rate limit HMAC compartido y picos excluidos. Fase 6 agregará mantenimiento diario auditable. `src/layouts/PublicLayout.tsx` permanece fuera del alcance. |
| 2026-07-14 22:50 | Codex | Finaliza y despliega Fases 5 y 6: commits `b53e3ff` y `fc84e1b` publicados en `main`; migraciones `20260715210000` y `20260715220000` aplicadas; secret HMAC configurado; `collect-analytics` versión 5 `ACTIVE`; cron diario y limpieza inicial confirmados por migración transaccional. Validó 83 tests, preflight, build, PWA, Deno, dry-run y lint. Netlify terminó `nyzca.com`, el enlace Supabase volvió a cuarentena y `PublicLayout.tsx` siguió excluido. |
