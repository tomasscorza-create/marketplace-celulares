-- Adds admin-managed spec templates per category (e.g. RAM, Memoria for
-- "Telefonos") and a per-product snapshot column to store filled values.
-- Recreates catalog RPCs so their return contracts include the new column.

create table public.category_spec_templates (
  id uuid primary key default gen_random_uuid(),
  category_id uuid not null references public.categories (id) on delete cascade,
  field_label text not null,
  sort_order integer not null default 0,
  created_at timestamptz not null default now(),
  unique (category_id, field_label)
);

create index category_spec_templates_category_id_idx
  on public.category_spec_templates (category_id);

alter table public.category_spec_templates enable row level security;

drop policy if exists category_spec_templates_select_authenticated on public.category_spec_templates;
create policy category_spec_templates_select_authenticated
  on public.category_spec_templates for select
  to authenticated
  using (true);

drop policy if exists category_spec_templates_write_admin_only on public.category_spec_templates;
create policy category_spec_templates_write_admin_only
...
alter table public.products
  add column if not exists category_spec_values jsonb not null default '[]'::jsonb;
  ...
  left join lateral (
    select jsonb_agg(
      jsonb_build_object(
        ...
        'category_spec_values', selected_products.category_spec_values,
