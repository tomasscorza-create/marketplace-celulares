-- Amplia la busqueda de catalogo para que tambien compare contra
-- product_attributes (dato interno, nunca expuesto en el detalle publico).
-- Asi una busqueda como "iphone rosa" encuentra una funda con
-- {key: "color", value: "rosa"} aunque el titulo no contenga esa palabra.
  ...
  scoped as (
    ...
    where p.is_active = true
      ...
      and (
        ...
        or exists (
          select 1
          from jsonb_array_elements(p.product_attributes) as attr
          where (attr ->> 'key') ilike '%' || n.search_text || '%'
             or (attr ->> 'value') ilike '%' || n.search_text || '%'
        )
      )
  )
  select
